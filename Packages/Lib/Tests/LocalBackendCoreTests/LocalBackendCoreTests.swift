...
@Suite(
    "LocalBackend Core Tests",
    .dependencies {
        $0.defaultDatabase = .observableModelDatabase(path: .inMemory)
        $0.backendService = LocalBackendService()
    }
)
class LocalBackendCoreTests {
    ...
    private func createTestScheme(projectId: String = "com.test.app")
        -> SchemeValue
    {
        SchemeValue(
            id: UUID(),
    ...
    func createScheme() async throws {
        ...
        #expect(
            fetchedScheme?.projectBundleIdentifier == project.bundleIdentifier
        )
    ...
    func fetchVersionsFromValidRepository() async throws {
        ...
        let repoURL = URL(
            string: "https://github.com/VisionaryLTD/Vision4.git"
        )!
        ...
        do {
            ...
            print(
                "fetchVersions failed (expected in some environments): \(error)"
            )
            #expect(
                Bool(false),
                "fetchVersions should not crash even if it fails"
            )
        }
    }
    ...
    func fetchBranchesFromValidRepository() async throws {
        ...
        do {
            ...
            // Just verify the method exists and can be called
            print(
                "fetchBranches failed (expected in some environments): \(error)"
            )
        }
    }
    ...
    func fetchVersionsFromInvalidRepository() async throws {
        ...
        let invalidRepoURL = URL(
            string:
                "https://github.com/this-definitely-does-not-exist-12345/repository.git"
        )!

        // The method should either throw an error or return empty array
        do {
            let versions = try await service.fetchVersions(
                remoteURL: invalidRepoURL
            )
            // If it succeeds, it should return empty array (no versions found)
            print(
                "fetchVersions returned \(versions.count) versions for invalid repo (this is acceptable)"
            )
        } catch {
    ...
    func fetchBranchesFromInvalidRepository() async throws {
        ...
        do {
            ...
            print(
                "fetchBranches returned \(branches.count) branches for invalid repo (this is acceptable)"
            )
        } catch {
            // Error is also acceptable - git command failed as expected
            print("Expected error for invalid repository: \(error)")
